...
#define FLAGSAPPEND O_RDWR|O_CREAT|O_APPEND 
    ...
    while(cout<<"% " && getline(cin, cmdline)){
        ...
        for(int v=0;v<S_C_B_N.size();v++){
            ...
            for(int i=0;i<tokens.size();i++){
                ...
                else if(tokens[i] == ">>"){  //要寫進檔案(append)
                    write2fileAPPEND=true;
                    commands.push_back(argments); //argments先push進argment
                    argments.clear();  //下一個argment存檔名，所以清空
                }
                else{
            ...
            if(pipetype == -1){
                ...
                for(int i = 0; i <= cmdcnt; i++){
                    ...
                    if(pid == 0){ //子程序
                        ...
                        }else if(write2fileAPPEND == true && i == cmdcnt){
                            int file;       // 開啟檔案後會回傳一個File Descriptor(檔案描述器)
                            if((file = open(exe_arg[0], FLAGSAPPEND, mode)) == -1){
                                cerr<<"Could not open file\n";
                                break;
                            }
